...
```
scp ~/Desktop/tinaasadi.txt pi@192.168.68.121:/home/pi/
```
...
```
put /Users/tinaasadi/Desktop/midtermPi.txt
```
...
```
It saves the current directory path to `mt.txt` and appends a directory tree structure to `pr.txt`.
```
...
```
sudo useradd -m -s /bin/bash TinaAsadi
```
...
```
sudo mkdir /home/TinaAsadi
sudo chown TinaAsadi:TinaAsadi /home/TinaAsadi
```
